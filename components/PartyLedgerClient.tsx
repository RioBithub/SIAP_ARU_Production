...
export default function PartyLedgerClient({kind,user}:{kind:Kind;user:SessionUser}){
  ...
  return <main className="page">
    ...
    {mode==="SUMMARY"&&<>
      {canEdit&&<details className="card finance-details-card" style={{marginBottom:16}}><summary><b>Edit Posisi Lengkap</b><span>Gunakan bila perlu mengubah beberapa angka sekaligus, jumlah item, tanggal posisi, atau catatan.</span></summary><div className="card-body"><div className="finance-position-form"><div className="field"><label>Posisi per Tanggal</label><input className="input" type="date" value={summaryForm.as_of_date} onChange={e=>setSummaryForm(f=>({...f,as_of_date:e.target.value}))}/></div><div className="field"><label>Total Outstanding</label><input className="input" type="number" min="0" value={summaryForm.outstanding_amount} onChange={e=>setSummaryForm(f=>({...f,outstanding_amount:e.target.value}))}/></div><div className="field"><label>Jatuh Tempo ≤30 Hari</label><input className="input" type="number" min="0" value={summaryForm.due_30_amount} onChange={e=>setSummaryForm(f=>({...f,due_30_amount:e.target.value}))}/></div><div className="field"><label>Jumlah Item ≤30 Hari</label><input className="input" type="number" min="0" value={summaryForm.due_30_count} onChange={e=>setSummaryForm(f=>({...f,due_30_count:e.target.value}))}/></div></div><div className="finance-aging-edit"><div className="field"><label>Aging 1-30</label><input className="input" type="number" min="0" value={summaryForm.aging_1_30_amount} onChange={e=>setSummaryForm(f=>({...f,aging_1_30_amount:e.target.value}))}/></div><div className="field"><label>Aging 30-60</label><input className="input" type="number" min="0" value={summaryForm.aging_30_60_amount} onChange={e=>setSummaryForm(f=>({...f,aging_30_60_amount:e.target.value}))}/></div><div className="field"><label>Aging 60-90</label><input className="input" type="number" min="0" value={summaryForm.aging_60_90_amount} onChange={e=>setSummaryForm(f=>({...f,aging_60_90_amount:e.target.value}))}/></div><div className="field"><label>Aging 90+</label><input className="input" type="number" min="0" value={summaryForm.aging_90_plus_amount} onChange={e=>setSummaryForm(f=>({...f,aging_90_plus_amount:e.target.value}))}/></div></div><div className="finance-overdue-preview"><span>Total overdue dari aging</span><b>{rupiah(agingTotal)}</b></div><div className="field" style={{marginTop:12}}><label>Catatan</label><textarea className="textarea" value={summaryForm.notes} onChange={e=>setSummaryForm(f=>({...f,notes:e.target.value}))}/></div><div style={{display:"flex",justifyContent:"flex-end",marginTop:12}}><button className="btn btn-primary" onClick={saveSummary} disabled={busy}>{busy?"Menyimpan...":"Simpan Posisi Lengkap"}</button></div></div></details>}
